...
# Transformado em default value e constante para fácil ajuste
DEFAULT_DATA_REMOVAL_RATIO = 0.0
data_removal_ratio = None


TIME_ENCODING_DIM = 16  # dimensão do vetor sen/cos de tempo
...
def positional_encoding_from_timestamps(
    timestamps: torch.Tensor, d_model: int
) -> torch.Tensor:
    """
    Gera codificação sen/cos a partir dos timestamps (em minutos).

    Args:
        timestamps: (batch, seq_len, 1) ou (batch, seq_len)
        d_model: dimensão da codificação (TIME_ENCODING_DIM)

    Returns:
        Tensor (batch, seq_len, d_model)
    """
    # Se vier como (B, L, 1), tira o último eixo
    if timestamps.dim() == 3:
        positions = timestamps.squeeze(-1)  # (B, L)
    else:
        positions = timestamps  # (B, L)

    # deixa como (B, L, 1) pra facilitar o broadcast
    positions = positions.unsqueeze(-1)  # (B, L, 1)
    ...
    # saída
    pe = torch.zeros(
        positions.size(0), positions.size(1), d_model, device=device, dtype=dtype
    )

    # canais pares: seno, ímpares: cosseno
...
class ARModel(nn.Module):
    """Autoregressive RNN Model using GRU + codificação temporal."""

    def __init__(self, input_size: int, hidden_size: int, time_encoding_dim: int):
        super().__init__()
        # input_size = número de features (ssh, etc.)
        self.feature_size = input_size
        self.time_encoding_dim = time_encoding_dim
        self.hidden_size = hidden_size

        # Encoder recebe features + codificação temporal
        self.encoder = nn.GRU(
            input_size + time_encoding_dim, hidden_size, batch_first=True
        )
        # Decoder recebe só codificação temporal dos timestamps futuros
        self.decoder = nn.GRU(time_encoding_dim, hidden_size, batch_first=True)

        # Projeta de volta para o espaço de features originais
        self.linear = nn.Linear(hidden_size, input_size)

    def _add_time_encoding_to_encoder_input(
        self,
        x: torch.Tensor,
        ...
        """
        Concatena codificação temporal ao input do encoder.

        x: (B, L, feature_size)
        x_timestamps: (B, L, 1)
        """
        pe = positional_encoding_from_timestamps(
            x_timestamps, self.time_encoding_dim
        )  # (B, L, time_encoding_dim)

        # concatena na dimensão de features
        x_with_time = torch.cat(
            [x, pe], dim=-1
        )  # (B, L, feature_size + time_encoding_dim)
        return x_with_time
    ...
    def encode(
        ...
        """Encoda a sequência passada (com tempo) em um hidden state."""
        x_packed = nn.utils.rnn.pack_padded_sequence(
            x_with_time, x_lengths.cpu(), batch_first=True, enforce_sorted=False
        )
        _, h_n = self.encoder(x_packed)  # (1, B, hidden_size)
        return h_n
    ...
    def decode(
        ...
        """Decodifica autoregressivamente usando apenas codificação temporal."""
        # codificação temporal para timestamps futuros
        pe = positional_encoding_from_timestamps(
            y_timestamps, self.time_encoding_dim
        )  # (B, L, time_encoding_dim)

        pe_packed = nn.utils.rnn.pack_padded_sequence(
            pe, y_lengths.cpu(), batch_first=True, enforce_sorted=False
        )

        out, _ = self.decoder(pe_packed, h_n)
        out, _ = nn.utils.rnn.pad_packed_sequence(out, batch_first=True)

        y_hat = self.linear(out)  # (B, L, feature_size)
        return y_hat
    ...
    def forward(
        ...
        """
        x: (B, Lx, feature_size)          passado
        x_timestamps: (B, Lx, 1)          timestamps do passado
        y_timestamps: (B, Ly, 1)          timestamps futuros
        """
        x_with_time = self._add_time_encoding_to_encoder_input(
            x, x_timestamps, x_lengths
        )
        h_n = self.encode(x_with_time, x_lengths)
        output_seq = self.decode(
            h_n=h_n, y_timestamps=y_timestamps, y_lengths=y_lengths)
        return output_seq
...
def run_train_epoch(
    ...
    for (
        ...
        # manda dados pro device
        inputs = input_features.to(device)  # (B, Lx, feat)
        inputs_timestamps = input_timestamps.to(device)  # (B, Lx, 1)
        targets = target_features.to(device)  # (B, Ly, feat)
        targets_timestamps = target_timestamps.to(device)  # (B, Ly, 1)

        optimizer.zero_grad()

        # corta para o tamanho máximo real da janela futura (por causa do padding)
        max_target_len = int(target_lengths.max().item())
        targets = targets[:, :max_target_len]
        targets_timestamps = targets_timestamps[:, :max_target_len]

        # Rebase dos timestamps por janela:
        # passado: relativo ao primeiro instante da janela (começa em 0)
        # futuro: relativo ao último instante observado do passado
        inputs_timestamps_rel = inputs_timestamps - inputs_timestamps[:, :1, :]
        targets_timestamps_rel = targets_timestamps - inputs_timestamps[:, -1:, :]

        # FORWARD PASS com codificação temporal
        outputs = model(
            inputs,
            inputs_timestamps_rel,
            input_lengths,
            targets_timestamps_rel,
            target_lengths,
        )
...
def run_eval_epoch(
    ...
    for (
        ...
        with torch.no_grad():
            ...
            targets_timestamps = target_timestamps.to(device)

            max_target_len = int(target_lengths.max().item())
            targets = targets[:, :max_target_len]
            targets_timestamps = targets_timestamps[:, :max_target_len]

            # Rebase dos timestamps por janela (mesma lógica do treino)
            inputs_timestamps_rel = inputs_timestamps - inputs_timestamps[:, :1, :]
            targets_timestamps_rel = targets_timestamps - inputs_timestamps[:, -1:, :]
            ...
            target_timestamps = [
                target_timestamps[i, : target_lengths[i]].cpu().numpy()
...
def plot_results(train_losses, test_losses, epoch, output_filename="loss_curve.png"):
    ...
    plt.close()
    print(f"Loss curve saved to {output_filename}")


def plot_prediction(model, X_test, y_test, device, past_len, future_len, idx=0, output_filename=None):
    ...
    # Plot past input
    plt.plot(range(past_len), input_seq_plot,
             label="Input (Past Data)", color="blue")
    ...
    if output_filename is None:
        output_filename = f"prediction_example_{idx}.png"
    plt.savefig(output_filename)
    plt.close()
    print(f"Prediction plot saved to {output_filename}")
...
def main(args):
    ...
    # ============================
    # Novo parâmetro para proporção de dados removidos
    # ============================
    global data_removal_ratio
    data_removal_ratio = args.data_removal_ratio
    print(f"Data removal ratio set to: {data_removal_ratio}")
    ...
    print(
        f"Scaling data using Train Mean: {train_mean}, Train Std: {train_std}")
    ...
    plot_results(train_losses, test_losses,
                 args.num_epochs, args.loss_plot_file)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Autoregressive RNN Training Script")
    parser.add_argument(
        "--past_len",
        type=int,
    ...
    # Novo argumento para definir a proporção de dados removidos
    parser.add_argument(
        "--data_removal_ratio",
        type=float,
        default=DEFAULT_DATA_REMOVAL_RATIO,
        help="Proportion of data to remove to simulate missing data (between 0 and 1).",
    )
    parser.add_argument(
        "--loss_plot_file",
        type=str,
